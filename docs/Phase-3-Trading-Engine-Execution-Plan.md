...
**Status:** Approved, with all 8 owner decisions in §25 accepted as recommended. Sprints 32–38 are shipped — see their entries in §21 for the as-built write-ups (Sprints 34/36/37 each shipped a Core module reordered ahead of a previously-scheduled Route+UI sprint; Sprint 38 shipped the Risk Management Engine's Core, chosen over the outstanding Route+UI backlog via the established `AskUserQuestion`-failure recovery — see each sprint's own as-built note for the disclosed deviation/decision). Five items are now unscheduled as a result (4 Route+UI sprints — Market Structure, Liquidity, Multi-Timeframe, Regime Detection — plus Risk Management's own Route+UI; see the roadmap table's Sprint 39 note). Sprint 39 onward is planning only until each sprint's own pre-implementation plan is separately approved, per the established per-sprint process (`CLAUDE.md` §3).
...
| 39 | *(unscheduled — outstanding backlog)* | Five items now sit unscheduled: 4 Route+UI sprints (Market Structure — originally Sprint 34, Liquidity — originally Sprint 36, Multi-Timeframe — originally Sprint 37, Regime Detection — originally Sprint 39) plus Risk Management's own Route+UI (§15's second half). The next unbuilt Core module (Trading Journal, §16) is also a candidate. Sprint 39's actual scope is to be decided at that sprint's own kickoff. |
