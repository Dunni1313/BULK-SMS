...
**Status:** Approved, with all 8 owner decisions in §25 accepted as recommended. Sprints 32–36 are shipped — see their entries in §21 for the as-built write-ups (Sprint 34 shipped the Multi-Timeframe Trend Engine's Core, reordered ahead of Market Structure's own Route+UI sprint; Sprint 36 shipped the Market Regime Detection Engine's Core, reordered ahead of Liquidity's own Route+UI sprint — both at the project owner's explicit direction, see each sprint's own as-built note for the disclosed deviation). Sprint 37 onward is planning only until each sprint's own pre-implementation plan is separately approved, per the established per-sprint process (`CLAUDE.md` §3).
...
| 37 | Multi-Timeframe Trend Engine (Route + UI) *(repurposed)* | Originally "Multi-Timeframe Engine" as a single sprint; Core shipped early at Sprint 34, so this slot now covers its route + UI. `GET /trading/multi-timeframe/:symbol`, confluence panel on the Trading Research page. **Market Structure's own Route+UI work (originally Sprint 34) and Liquidity's own Route+UI work (originally Sprint 36) are both still outstanding and unscheduled** — candidates for this slot, Sprint 39's slot, or a later sprint, to be decided at that sprint's own kickoff. |
| 38 | Probability Engine | Historical-volatility-based probability cone, honest-unavailable paths. |
| 39 | Regime Detection (Route + UI) *(repurposed)* | Originally "Regime Detection" as a single sprint; Core shipped early at Sprint 36, so this slot now covers its route + UI. `GET /trading/regime/:symbol`, regime badge/panel on the Trading Research page. |
