...
**Status:** Approved, with all 8 owner decisions in §25 accepted as recommended. Sprints 32–37 are shipped — see their entries in §21 for the as-built write-ups (Sprints 34/36/37 each shipped a Core module reordered ahead of a previously-scheduled Route+UI sprint, at the project owner's explicit direction each time — see each sprint's own as-built note for the disclosed deviation). Four Route+UI sprints are now unscheduled as a result (Market Structure, Liquidity, Multi-Timeframe, Regime Detection — see the roadmap table's Sprint 38/39 notes). Sprint 38 onward is planning only until each sprint's own pre-implementation plan is separately approved, per the established per-sprint process (`CLAUDE.md` §3).
...
| 38 | *(unscheduled — outstanding Route+UI backlog)* | Four Route+UI sprints now sit unscheduled from prior reorderings: Market Structure's own (originally Sprint 34), Liquidity's own (originally Sprint 36), Multi-Timeframe's own (originally Sprint 37), and Regime Detection's own (originally Sprint 39). Sprint 38's actual scope — one of these, a combined Route+UI sprint covering several at once, or a further Core module — is to be decided at that sprint's own kickoff. |
| 39 | *(unscheduled)* | See Sprint 38's note; whichever Route+UI/Core work isn't covered at Sprint 38 remains a candidate here. |
