...
## Sprints 2–5 — As-Built (Frontend UI, COMPLETE)

Per the project owner's explicit direction, Sprints 2 (persistence/history
— already largely covered by Sprint 1's `GET /trading-coach/messages`) and
3–5 (frontend panel, context chips, suggested actions) were implemented
together as one UI sprint on branch `v1.3.1-ai-trading-coach-ui`, reusing
the Sprint 1 backend contract exactly as documented — **zero backend
files were changed** (`git diff --stat` against every `artifacts/api-server`,
`lib/db`, `lib/api-spec`, `lib/api-zod`, and `lib/api-client-react` path is
empty).

### Pre-coding audit (performed before writing any component)

Confirmed via direct inspection: `streamCoach()` (`src/lib/coach-stream.ts`)
is a generic, endpoint-agnostic SSE client already proven against 5
existing embedded coach panels — reused verbatim, unmodified. `Markdown`
(`components/ui/markdown.tsx`) already renders assistant prose with clear
heading/list/table hierarchy — reused verbatim for every assistant turn.
`pages/Assistant.tsx`'s pending-user/streaming-reply/error/stop state
machine (Sprint 59) is the closest existing precedent for a
server-persisted, streaming chat surface and was used as the direct model
for `TradingCoachWorkspace`'s own send/stream/error/stop logic — not
copied, but the same shape, so the safety/UX guarantees it already proved
(never silently drops a failed reply, stopping never discards the partial
answer) carry over identically. `TradingCoachDrawer`/`CoachDrawer`
(Phase 29 / Phase 21's deterministic, zero-LLM explainer drawers) were
confirmed to be a **different, already-shipped feature** — reused as-is,
untouched, and deliberately not duplicated or replaced.

### What was built

A dockable Sheet panel (reachable from a persistent header launcher, next
to `NotificationBell`) **and** a permanent sidebar-nav-reachable full page
— both render the exact same `TradingCoachWorkspace` component, so there
is only one implementation of the Coach's actual behavior:

- `hooks/use-trading-coach.tsx` — `TradingCoachProvider`/`useTradingCoach()`,
  a thin React Context (mounted once in `App.tsx`, wrapping `<AppLayout>`)
  holding only the panel's open/closed state and the current `focus`
  (optional `symbol`/`scannerCandidateId`/`scannerCandidateLabel`/
  `tradingPositionId`/`tradingPositionLabel`) — never conversation history
  or streaming state, which stays server-persisted/component-local exactly
  like every other coach panel in this codebase.
- `lib/trading-coach-context.ts` — the shared context adapters
  (`focusFromSymbol`/`focusFromScannerCandidate`/`focusFromTradingPosition`)
  and the two static rule tables (`getSuggestedPrompts`/`getSuggestedActions`)
  — the only place any page-specific shape (a Scanner row, a Trade
  Execution Center candidate, a Trading Positions row) is known, so the
  Coach components themselves stay page-agnostic.
- `components/coach/` — `TradingCoachLauncher` (header icon button),
  `TradingCoachPanel` (the Sheet wrapper, lazy-loaded like `CommandPalette`),
  `TradingCoachWorkspace` (the orchestrator — owns every hook: persisted
  history via the generated `useGetUnifiedTradingCoachMessages()`, and the
  send/stream/stop/error/retry state mirroring `Assistant.tsx`'s own
  pattern), `TradingCoachHeader`, `TradingCoachContextPanel` (the "Viewing"
  route label + "Shared with Coach" chip row — only chips that genuinely
  apply, plus the two always-on Trading Positions Risk / Options Portfolio
  signals `buildUnifiedCoachContext()` composes regardless of focus, plus
  a Clear-context action for the optional focus), `TradingCoachMessages`
  (history/streaming/loading/empty/error rendering via the shared
  `Markdown` component), `TradingCoachSuggestedPrompts` (quick-fill
  chips), `TradingCoachSuggestedActions` (post-answer deep-link chips —
  `wouter` navigation only, to `/ticket/:id`, `/trading-research`,
  `/options-dashboard`, `/portfolio-ai` — never a mutation call),
  `TradingCoachComposer` (input/send/stop, plus a "Clear conversation"
  action gated behind an `AlertDialog` confirmation whose copy is explicit
  that it clears the *local view only* — Sprint 1's backend has no
  delete/clear endpoint, and none was added), `TradingCoachDisclaimer`
  (a concise, always-visible advisory footer — distinct from, and in
  addition to, the `COACH_DISCLAIMER` text `lib/ai-core`'s `narrate()`/
  `narrateStream()` already appends into every answer server-side),
  `TradingCoachEmptyState`, `TradingCoachErrorState`.
- `pages/AITradingCoach.tsx` — the sidebar-nav entry point (new
  `/ai-trading-coach` route, `AI & Decision Tools` group, `MessageCircle`
  icon), a thin page shell around the identical `TradingCoachWorkspace`.

### Integration points (per the design doc's own §9 table)

- **Scanner** (`Scanner.tsx`) — a new `MessageCircle` icon button per row,
  alongside the pre-existing `TradeExplanationSheet` trigger, opening the
  Coach with `focusFromScannerCandidate(result)`.
- **Trade Execution Center** (`TradeExecutionCenter.tsx`) — a header
  trigger, visible once a candidate is `selected`, opening the Coach with
  that same candidate's focus.
- **Options Dashboard** (`Dashboard.tsx`) and **Portfolio** (`Portfolio.tsx`)
  — an always-available header trigger with no specific focus, since
  `buildUnifiedCoachContext()` always composes the options-income
  portfolio/dashboard signal regardless.
- **Market Structure Engine / Institutional Trading** (`TradingResearch.tsx`)
  — a header trigger (visible once a symbol is searched) using
  `focusFromSymbol()`, plus a per-position trigger in the existing
  Portfolio Risk section using `focusFromTradingPosition()`. The page's
  own pre-existing, separate embedded coach panel (Sprint 47/48's
  per-page `/trading/coach/ask/stream`) is completely untouched — the new
  trigger opens the platform-wide Coach as an additional, clearly distinct
  surface.
- **Institutional Investing (Engine 1)** — **deliberately not integrated**,
  matching this design doc's own §14 explicit out-of-scope declaration;
  `buildUnifiedCoachContext()` has no Engine 1 read path at all, so wiring
  a trigger into an Investing page without backend support would imply
  context the Coach cannot actually see. The header launcher/sidebar page
  remain reachable from any Investing page for general questions.

### Safety controls verified

Every requirement in the approved design's §8 holds: no code path in any
new component calls `execution.ts`/`optionsMath.ts`/`risk.ts`/
`autoExecution.ts`/`autoAdjustment.ts`, or any mutation hook at all —
`TradingCoachSuggestedActions` renders `wouter` `<Link>`s exclusively,
proven by a dedicated test asserting no `<form>`/mutation trigger exists
inside that component. `COACH_DISCLAIMER` continues to be enforced
centrally and unconditionally, unmodified. `TradingCoachDisclaimer`
states plainly the Coach never places/modifies/closes an order and cannot
enable live trading. Every response-structure claim (Observation/
Explanation/Risks/Questions to consider/Educational next step) is
disclosed as advisory formatting only — the underlying narration prompt
is unmodified this sprint, so the Coach's own prose determines whether
that structure appears; `Markdown`'s existing heading hierarchy renders it
clearly when it does, never fabricated or enforced client-side.

### Files created

`hooks/use-trading-coach.tsx` (+`.test.tsx`), `lib/trading-coach-context.ts`
(+`.test.ts`), `components/coach/TradingCoachLauncher.tsx`,
`TradingCoachPanel.tsx` (+`.test.tsx`), `TradingCoachWorkspace.tsx`
(+`.test.tsx`), `TradingCoachHeader.tsx`, `TradingCoachContextPanel.tsx`
(+`.test.tsx`), `TradingCoachMessages.tsx`, `TradingCoachSuggestedPrompts.tsx`
(+`.test.tsx`), `TradingCoachSuggestedActions.tsx` (+`.test.tsx`),
`TradingCoachComposer.tsx` (+`.test.tsx`), `TradingCoachDisclaimer.tsx`,
`TradingCoachEmptyState.tsx`, `TradingCoachErrorState.tsx`
(+`TradingCoachPresentational.test.tsx` covering the last 3),
`pages/AITradingCoach.tsx` (+`.test.tsx`).

### Files modified

`App.tsx` (mount `TradingCoachProvider`, lazy-load + route the new page),
`components/layout/AppLayout.tsx` (mount `TradingCoachLauncher`/`Panel`),
`lib/nav-items.ts` (+`.test.ts`, new `/ai-trading-coach` entry),
`test/test-utils.tsx` (`renderWithClient` now also wraps
`TradingCoachProvider`, since it's a permanent part of the real app shell —
the single, correct place to fix every affected page test at once rather
than adding a per-file wrapper), `pages/Scanner.tsx`, `TradeExecutionCenter.tsx`,
`Dashboard.tsx`, `Portfolio.tsx`, `TradingResearch.tsx` (integration
triggers) and their own `.test.tsx` files (one new, targeted assertion
each — full Coach behavior is exhaustively covered in
`TradingCoachWorkspace.test.tsx` instead of being re-tested per page),
`components/layout/AppLayout.test.tsx`/`.mobile.test.tsx` (launcher +
sidebar-nav reachability, expanded/collapsed/mobile).

### Validation

`pnpm run typecheck` — clean across all 5 workspace packages.
`pnpm --filter @workspace/ravish-trading run test` — **109 files / 1,246
tests, all passing** (was 99/1,158 before this sprint — 88 new tests: 10
new coach-focused test files plus one new targeted assertion in each of
the 7 modified existing test files). `PORT=5000 BASE_PATH=/ pnpm run build`
— all packages build successfully; `TradingCoachWorkspace` compiles into
its own separate, lazy-loaded chunk (confirmed in the build output), not
the eagerly-loaded main bundle. No backend test run was required — zero
backend files changed.

**Sprints 2–5 are complete. Awaiting review before Sprint 6
(documentation + final validation pass) or a pull request.**
