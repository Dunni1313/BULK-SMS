...
export async function loadExecutiveIntelligenceInputs(userId: string) {
  const [investingInputs, tradingInputs, reportRows, notificationRows, journalRowsForTimeline] = await Promise.all([
  ...
  return { investingInputs, tradingInputs, reportRows: reports, activityInput };
}
