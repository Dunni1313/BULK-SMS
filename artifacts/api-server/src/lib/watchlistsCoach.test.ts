...
import { WATCHLISTS_COACH_TOPICS, explainWatchlistsTopic, allWatchlistsTopics } from "./watchlistsCoach.js";
import { COACH_DISCLAIMER } from "./coach.js";

describe("explainWatchlistsTopic", () => {
  it("returns a well-shaped explanation for every one of the 5 named topics", () => {
    for (const topic of WATCHLISTS_COACH_TOPICS) {
  ...
  it("always carries the same, unmodified COACH_DISCLAIMER text every other coach domain uses", () => {
    for (const topic of WATCHLISTS_COACH_TOPICS) {
      expect(explainWatchlistsTopic(topic)!.disclaimer).toBe(COACH_DISCLAIMER);
    }
  });

  it("returns null for an unknown topic, never a fabricated explanation", () => {
    expect(explainWatchlistsTopic("not_a_real_topic")).toBeNull();
    expect(explainWatchlistsTopic("")).toBeNull();
  });

  it("never recommends a trade, a hedge, or a specific symbol — structurally, since no function here accepts one", () => {
    // explainWatchlistsTopic's own signature takes only a topic key (a
    ...
    // explanation text.
    for (const topic of WATCHLISTS_COACH_TOPICS) {
      const text = explainWatchlistsTopic(topic)!.explanation.join(" ").toLowerCase();
...
describe("allWatchlistsTopics", () => {
  it("returns exactly the 5 named topics, in order, each matching its own single-topic lookup", () => {
    const all = allWatchlistsTopics();
    expect(all).toHaveLength(5);
    expect(all.map((e) => e.topic)).toEqual([...WATCHLISTS_COACH_TOPICS]);
    for (const entry of all) {
      expect(entry).toEqual(explainWatchlistsTopic(entry.topic));
    }
  });
});
