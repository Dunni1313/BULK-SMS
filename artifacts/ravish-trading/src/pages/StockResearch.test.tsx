...
describe("ReportView", () => {
  ...
  describe("v1.5.0 Sprint 10 — Institutional Trade Planner", () => {
    it("trade plans are collapsed by default; the toggle reveals the trade plan sidebar", async () => {
      const report = makeValueReport();
      renderWithClient(<ReportView report={report} commentary="" isStreaming={false} />);
      expect(screen.queryByTestId("investing-trade-plan-sidebar")).not.toBeInTheDocument();

      await userEvent.click(await screen.findByTestId("button-toggle-investing-trade-plans"));
      expect(await screen.findByTestId("investing-trade-plan-sidebar")).toBeInTheDocument();
    });
    ...
    it("creating a trade plan adds it to the sidebar, and selecting it shows the header/editor/checklist/summary panel", async () => {
      ...
      await userEvent.click(await screen.findByTestId("investing-trade-plan-sidebar-new-plan"));
      await userEvent.type(screen.getByTestId("investing-trade-plan-sidebar-create-title"), "Value thesis trade plan");
      await userEvent.click(screen.getByTestId("investing-trade-plan-sidebar-create-save"));

      const card = await screen.findByTestId(/investing-trade-plan-sidebar-list-card-\d+-select/);
      await userEvent.click(card);

      expect(await screen.findByTestId("investing-trade-plan-header")).toHaveTextContent("Value thesis trade plan");
      expect(screen.getByTestId("investing-trade-plan-editor")).toBeInTheDocument();
      expect(screen.getByTestId("investing-trade-plan-checklist")).toBeInTheDocument();
      expect(screen.getByTestId("investing-trade-plan-summary-panel")).toBeInTheDocument();
    });
  });
});
